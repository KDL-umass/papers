...
def regression_loss(concat_true, concat_pred, binary_flag = False):
    ...
    if binary_flag:
        loss0 = tf.reduce_sum((1. - t_true) * K.binary_crossentropy(y_true, y0_pred, from_logits= True))
        loss1 = tf.reduce_sum(t_true * K.binary_crossentropy(y_true, y1_pred, from_logits= True))
    else:
...
def dragonnet_loss_binarycross(concat_true, concat_pred, binary_flag = False):
    return regression_loss(concat_true, concat_pred, binary_flag = binary_flag) + binary_classification_loss(concat_true, concat_pred)
...
def make_tarreg_loss(ratio=1., dragonnet_loss=dragonnet_loss_binarycross, binary_flag = False):
    def tarreg_ATE_unbounded_domain_loss(concat_true, concat_pred):
        vanilla_loss = dragonnet_loss(concat_true, concat_pred, binary_flag=binary_flag)
        ...
        if binary_flag:
            targeted_regularization = tf.reduce_sum(K.binary_crossentropy(y_true, y_pert, from_logits= True))
        else:
...
def make_dragonnet(input_dim, reg_l2, binary_flag = False):
    """
...
def make_tarnet(input_dim, reg_l2, binary_flag = False):
    """
    Neural net predictive model. The dragon has three heads.
    :param input_dim:
    :param reg:
